...
#ifndef SCHED_LOCAL_THREADS_PROGRAMMABLE_HPP
#define SCHED_LOCAL_THREADS_PROGRAMMABLE_HPP
...
class programmable_local: public sched::base,
...
protected:
   ...
   virtual void change_node(thread_node *, programmable_local *);
...
public:
   ...
   programmable_local *find_scheduler(const db::node *);
   
   static std::vector<sched::base*>& start(const size_t);
   
   explicit programmable_local(const vm::process_id);
   
   virtual ~programmable_local(void);
};
