...
private:
   ...
   mutable std::vector< utils::atomic<int> > to_proc;
   // counters with numbers of tuples generated by the node itself
   mutable std::vector<int> auto_generated;
   ...
   inline void new_auto_tuple(const simple_tuple *stpl) const
   {
      more_to_process(stpl->get_predicate_id());
      auto_generated[stpl->get_predicate_id()]++;
   }
   
   inline bool no_more_to_process(const vm::predicate_id id) const { return to_proc[id] == 0; }
   inline void more_to_process(const vm::predicate_id id) const { assert(to_proc[id] >= 0); to_proc[id]++; }
   inline void less_to_process(const vm::predicate_id id) const { assert(to_proc[id] >= 1); to_proc[id]--; }
